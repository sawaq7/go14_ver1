package html5

   const General_index = `
   <!DOCTYPE html>
   <html>
	 <head>
		<meta charset="utf-8">
		<title>menu list</title>
		<link rel="stylesheet" href="css/sgh/tokura_index2.css" type="text/css">
	 </head>
	 <body>
	    <header>

	    </header>
		<nav>

			<ul>
			    <li>
			      <div>
			        <form method="GET"   >

			          <input type="image" src="images/tokura/shaberu.png" width="200" height="200"
                                                                           name="List" alt="png" />
			        </form>
			      </div>
			    </li>


			</ul>

		</nav>
		<article>
        </article>

		<section id="main">
		  <ul>
			    <li>
			      <div>
			        <h1  align="center">** storage **</h1>
			        <form method="GET"  action="/storage_bucket_list" >

			          <input type="image" src="images/illust_members_nakamura2.png" width="80" height="80"
                                                                           name="List" alt="png" />
			        </form>
			      </div>
			    </li>

                <li>
				  <div>

                    <h1  align="center">** datastore **</h1>
				    <form method="GET"  action="/datastore_copy_list_keyin" >
                      <input type="image" src="images/illust_finger.png" width="80" height="80"
                                                                           name="List" alt="png" />
                    </form>
                  </div>
                </li>

				<li>
				  <div>
				  <h1  align="center">none</h1>
				  <form method="GET"  action="/private_showall1" >
				      <input type="image" src="images/illust_finger.png" width="80" height="80"
                                                                           name="List" alt="png" />

                    </form>
                  </div>
				</li>
				<li>
				  <div>
				  <h1  align="center">none</h1>
				  <form method="GET"  action="/pipe_line1_show" >

				      <input type="image" src="images/illust_finger.png" width="80" height="80"
                                                              name="List" alt="png" />

                    </form>
                  </div>
				</li>
				<li>
				  <div>
				  <h1  align="center">none</h1>
				  <form method="GET" action="/pipe_line_ds_keyin" >
				      <input type="image" src="images/illust_finger.png" width="80" height="80"
                                                                           name="List" alt="png" />

                  </form>
                  </div>
				</li>

				<li>
				  <div>
				  <h1  align="center">none</h1>
				    <form method="GET"  action="/water_slope_show" >
				      <input type="image" src="images/illust_finger.png" width="80" height="80"
                                                                           name="List" alt="png" />

                    </form>
                  </div>
                </li>

				<li>

				</li>
			</ul>

		</section>
		<aside>

		</aside>
	</body>
  </html>
`

