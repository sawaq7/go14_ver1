       ...
       <section>
            <form method="GET" action="/d_schedule_showall" >
              ...
              {{range .}}
                {{ if eq .Id 77}}
                  <tr>
                       <th> Car-No.{{.Car_Name_01|html}} </th> <th>Car-No.{{.Car_Name_02|html}} </th>  <th>Car-No.{{.Car_Name_03|html}} </th>
                       <th>Car-No.{{.Car_Name_04|html}} </th> <th>Car-No.{{.Car_Name_05|html}} </th> <th>access1</th>
                  </tr>
                {{end}}
              {{end}}
              <tr>
       ...
       <section id="main">
           ...
           {{range .}}
             {{ if eq .Id 77}}
               ...
               <tr> <th>district-no</th> <th>date</th>  <th> Car-No.{{.Car_Name_01|html}} </th> <th>Car-No.{{.Car_Name_02|html}} </th>  <th>Car-No.{{.Car_Name_03|html}} </th>
                                                        <th>Car-No.{{.Car_Name_04|html}} </th> <th>Car-No.{{.Car_Name_05|html}} </th>
